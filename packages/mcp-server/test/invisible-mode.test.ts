...
test("invisible mode: session is absent from presence.who and emits no user.focused/user.joined to others", async () => {
  ...
  try {
    ...
    // Give the (would-be) debounced broadcast time to fire if it were going to.
    await new Promise((r) => setTimeout(r, 50))
    expect(bobSse.events.some((e) => e.kind === "user.focused")).toBe(false)
    expect(bobSse.events.some((e) => e.kind === "user.joined")).toBe(false)
    ...
    // Bob (visible) still shows up normally once he focuses too.
