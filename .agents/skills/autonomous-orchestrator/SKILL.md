...
## Step 0 — Scope Check (ONLY permitted pause)
...
For each required product file in `docs/product/`, if it does not already exist, copy it from the template model located in `.agents/skills/autonomous-orchestrator/models/`:

| File | Initial State & Copy Source |
|---|---|
| `docs/product/BACKLOG.md` | Copy from `.agents/skills/autonomous-orchestrator/models/BACKLOG.md` (then populate with features from step 1.2) |
| `docs/product/DEVELOPMENT-STATE.md` | Copy from `.agents/skills/autonomous-orchestrator/models/DEVELOPMENT-STATE.md` |
| `docs/product/DECISIONS.md` | Copy from `.agents/skills/autonomous-orchestrator/models/DECISIONS.md` |
| `docs/product/BOOTSTRAP-CONFIG.json` | Copy from `.agents/skills/autonomous-orchestrator/models/BOOTSTRAP-CONFIG.json` (substituting collected score thresholds and max reworks) |
...
1. `BACKLOG.md[feature]` → `Status: FAILED`
...
inputs:
  context = summary of changes made in completed cycle:
    ...
    - For each COMPLETED feature: ${domain}, scope description (Title + Description from BACKLOG.md), and paths created/modified
    - Final scores (TL + Adv) per COMPLETED feature — internal record only
    - Key decisions logged in DECISIONS.md this cycle
    - Current ${completedCycles} value
  instructions:
    - REQUIRED: All cross-references in generated docs MUST point ONLY to ./docs/adr/ or ./docs/feature/ folders
    - REQUIRED: Document the developed feature under docs/feature/{domain}.md (create if missing, update if exists)
    - REQUIRED: FOLDER STRUCTURE section → reflect current relevant structure
    - REQUIRED: Keep content direct and minimal
    - PROHIBITED: Narrative explanations, justifications, or process history
    - IF cycle introduced architectural changes → update corresponding docs/adr/*.md
    - PROHIBITED: Creating a new ADR file unless explicitly decided by a human
```
